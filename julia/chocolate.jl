...
# ╔═╡ 95f78e10-7bec-11ec-3ca0-affec2b5c2e9
begin
	using Gadfly, DataFrames, DataFramesMeta, CSV, Chain , CategoricalArrays
	using Turing, Random , Statistics, Distributions, MCMCChains, Optim
	Random.seed!(1020)
end

# ╔═╡ a34aa856-fd4c-496e-a109-749f569d44f2
begin
df = DataFrame(
CSV.File(download("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2022/2022-01-18/chocolate.csv", 
	"../data/chocholate.csv")))
end

# ╔═╡ 0aeff763-7c37-4c3f-9f79-7ced3ca98589
begin
	df_filt = @chain df begin 
		groupby(:country_of_bean_origin)
		@combine begin 
			counts = length(:country_of_bean_origin)
			ratings = :rating
		end
		@subset(:counts .>= 10)
		@select($(Not(:counts)))
...
# ╔═╡ 7903fa65-2d76-44d8-b22d-d3cc76de3339
begin
	nice_theme = Theme(major_label_font = "Cascadia Mono", 
		minor_label_font = "Cascadia Mono", 
		key_position = :none,
	)
	Gadfly.with_theme(nice_theme) do
		Gadfly.plot(df_filt, x = :country_of_bean_origin, y = :ratings, 
		color = :country_of_bean_origin, Geom.boxplot(), 
			Guide.xlabel("Country of bean origin"), 
			Guide.ylabel("Ratings"), 
			Coord.cartesian(ymin=0.5, ymax = 5)
		)
	end
end
...
s_ratings = filter(:country_of_bean_origin => ==("U.S.A."), df_filt)[!,:ratings]
...
begin
	...
	ϵ = 0.05
	τ = 10 
	chains = mapreduce(c -> sample(ratings_mean(s_ratings), NUTS(1000,0.65), iter), 
...
begin
	chain_layer = layer(x = chain_df[!,:μ], Geom.density, Theme(default_color = "red"))
	data_layer = layer(x = s_ratings, Geom.density, Theme(default_color = "steelblue"))
	plot(chain_layer, data_layer)
end
